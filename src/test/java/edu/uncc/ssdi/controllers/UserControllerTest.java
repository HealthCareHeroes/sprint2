...
public class UserControllerTest {
	...
	public void verifyGetUser() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get("/getUser/3").accept(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$.id").exists()).andExpect(jsonPath("$.email").exists())
				.andExpect(jsonPath("$.password").exists()).andExpect(jsonPath("$.id").value(3)).andDo(print());
	}

	@Test
	public void verifyUpdateUser() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.patch("/updateUser/3").contentType(MediaType.APPLICATION_JSON)
				.content("{ \"id\": \"3\", \"email\" : \"sample244@uncc.edu\", \"password\" : \"checkNew123\" }")
				...
				.andExpect(jsonPath("$.id").value(3)).andExpect(jsonPath("$.text").value("sample@uncc.edu"))
				.andExpect(jsonPath("$.completed").value("check123")).andDo(print());
	}
} // end of class